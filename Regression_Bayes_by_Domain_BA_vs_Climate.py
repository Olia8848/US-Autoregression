# -*- coding: utf-8 -*-
"""
Created on Fri Oct  4 13:22:41 2019
...
def getClimaticFactorsforRegress(df, Domain, NSIMS):
...
bio = getClimaticFactorsforRegress(df, Domain, NSIMS)
...
for j in range(19):
    if (j == cl1 or j == cl2 or j == cl3):
        R2results_for_clim_vars4 = numpy.append(R2results_for_clim_vars4, -1)
        continue
    X = dataset.iloc[:, [cl1+1, cl2+1, cl3+1, j+1]].values
...
cl4 = numpy.argmax(R2results_for_clim_vars4)
...
for j in range(19):
    if (j == cl1 or j == cl2 or j == cl3 or j == cl4):
        R2results_for_clim_vars5 = numpy.append(R2results_for_clim_vars5, -1)
        continue
    X = dataset.iloc[:, [cl1+1, cl2+1, cl3+1, cl4+1, j+1]].values
...
cl5 = numpy.argmax(R2results_for_clim_vars5)

print(cl1+1, cl2+1, cl3+1, cl4+1, cl5+1)
