...
AllUSA = ['211', '212', '221', '222', '223', '231', '232', '234', '242',
       '251', '255', '261', '262', '263', '313', '315', '321', '322',
       '331', '332', '341', '342', '411', 'M211', 'M221', 'M223', 'M231',
       'M242', 'M261', 'M262', 'M313', 'M331', 'M332', 'M333', 'M334',
       'M341']
...
SnapYear = 2000
SnapShotLocations = numpy.array([]) 
yearsOnPatch = numpy.array([])
loc=0

for observ in range(NOBSERV-1):
    
     if patch[observ] == patch[observ+1]:
          yearsOnPatch = numpy.append(yearsOnPatch, year[observ])
          continue
...
#          print(yearsOnPatch)
          ...
          index = (numpy.abs(yearsOnPatch-SnapYear)).argmin()
#          print('closest year:', yearsOnPatch[index])
          
#          print(loc, index, 'SnapShotLoc=', loc+index)
          SnapShotLoc = loc+index
          SnapShotLocations = numpy.append(SnapShotLocations, SnapShotLoc) 
#          print(SnapShotLocations)
          loc = loc + numpy.size(yearsOnPatch)
          yearsOnPatch = numpy.array([])
          continue

SnapShotLocations = numpy.array(SnapShotLocations)
SnapShotLocations = SnapShotLocations.astype(int)


df3 = df2.iloc[SnapShotLocations, :]
data = df3.values
...
climvars = pandas.DataFrame({'Annual Mean Temperature': bio1,
                            ...
                            'Precipitation of Warmest Quarte': bio18,
                            'Precipitation of Coldest Quarter': bio19}) 

import seaborn as sns
# calculate the correlation matrix
climvars = climvars.astype(float)
cl = climvars.corr()
