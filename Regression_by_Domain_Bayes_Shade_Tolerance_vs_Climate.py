# -*- coding: utf-8 -*-
"""
Created on Sun Nov  1 13:08:28 2020
...
def getClimaticFactorsforRegress(df, NSIMS):
...
dfEco = df[numpy.isin(ecoreg, Domain)]

# dfEcoShTol = dfEco[(dfEco[['Sc.DB']] != 0).all(axis=1)]
# numpy.shape(dfEco)[0]- numpy.shape(dfEcoShTol)[0]



bio = getClimaticFactorsforRegress(dfEco, NSIMS)
#bio = getClimaticFactorsforRegress(dfEco, NSIMS)

#data = dfEcoShTol.values
data = dfEco.values
NOBSERV = numpy.size(data, 0)
...
for j in range(19):
    if (j == clvar1 or j == clvar2 or j == clvar3):
        R2results_for_clim_vars4 = numpy.append(R2results_for_clim_vars4, -1)
        continue
...
for j in range(19):
    if (j == clvar1 or j == clvar2 or j == clvar3 or j == clvar4):
        R2results_for_clim_vars5 = numpy.append(R2results_for_clim_vars5, -1)
        continue
...
print(climvars[numpy.argmax(R2results_for_clim_vars)], ',',
      round(numpy.max(R2results_for_clim_vars)*100, 1), '%')

print(climvars[numpy.argmax(R2results_for_clim_vars2)], ',',
      round(numpy.max(R2results_for_clim_vars2)*100, 1), '%')

print(climvars[numpy.argmax(R2results_for_clim_vars3)], ',',
      round(numpy.max(R2results_for_clim_vars3)*100, 1), '%')

print(climvars[numpy.argmax(R2results_for_clim_vars4)], ',',
      round(numpy.max(R2results_for_clim_vars4)*100, 1), '%')

print(climvars[numpy.argmax(R2results_for_clim_vars5)], ',',
      round(numpy.max(R2results_for_clim_vars5)*100, 1), '%')
